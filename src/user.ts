...
import { ObjectId, AuthProvider } from ".";
...
export async function authenticate({
    ...
    if (provider && providerData)  {
        $filter['devices.id'] = providerData.id;
        $filter['devices.platform'] = provider;

        $set['isAnonymous'] = false;

        if (providerData.avatarUrl) {
            $set['avatarUrl'] = providerData.avatarUrl;
        }

        if (providerData.email) {
            $setOnInsert['email'] = providerData.email;
        }

        if (providerData.username) {
            $setOnInsert['username'] = providerData.username;
        }

        if (providerData.displayName) {
            $setOnInsert['displayName'] = providerData.displayName;
        }

        if (providerData.friendIds) {
            friendIds = (await User.
                find({ 'devices.id': { $in: providerData.friendIds } }, ["_id"])).
                map(user => user._id);
        }
        ...
        // facebook auth
        const data = await providers.facebook({ access_token: accessToken });
        ...
        if (data.displayName) {
            $setOnInsert['displayName'] = data.displayName;
        }
        ...
        if (data.friendIds) {
            facebookFriendsIds = data.friendIds;
        }
